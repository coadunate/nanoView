...
def plot_squiggle(args, filename, start_times, mean_signals):
    ...
    events_per_facet = (num_events / 1) + 1
...
def do_plot_squiggle(args, fast5, begin, end):
    start_times = []
    mean_signals = []
...
def run(parser, args):
    ...
    for fast5 in fast5_set:
        ...
        if first_fast5 is not None:
            do_plot_squiggle(args, first_fast5, 111.0, 112.0)
            first_fast5 = None
        do_plot_squiggle(args, fast5, 111.0, 112.0)
